...
public class Metamorphose extends AbstractBaseCard {
    public static final String ID = ModInformation.makeID(Metamorphose.class.getSimpleName());
        ...
        ActionHelper.addToBotAbstract(() -> {
            for (AbstractPower po : p.powers) {
                if (po instanceof AbstractFusionPower) {
                    this.addToBot(new ApplyPowerAction(p, p, po, po.amount));
                }
            }
        });
